...
Attributes:
    board (list): list of lists that repr the chessboard.
    solutions (list): A list of lists having solutions.

Solutions are repr in this format [[r, c], [r, c], [r, c], [r, c]]
where `r` and `c` repr the row and column, respectively, where a
...
def xout(board, row, col):
    ...
    # X out of * forward spots
    for c in range(col + 1, len(board)):
        board[row][c] = "x"

    # X out of * backwards spots
...
def recursive_solve(board, row, queens, solutions):
    """ To Recursively solve an N-queens puzzle.
